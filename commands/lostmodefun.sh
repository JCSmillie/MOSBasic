...
WHOISLOST() {
	...
	# ### WE NEED CODE TO SUPPORT PAGING HERE TO SCAN THROUGH ALL THE POSSIBLE PAGES
	# #### THEN GREP OUT WHO IS WAITNG TO ENABLE, WAITING TO DISABLE, AND HAS IT ENABLED.
	# #Build Query.  Just asking for current data on last beat, lostmode status, and location data if we can get it.
	# content="{\"accessToken\":\"$APIKey\",\"options\":{\"os\":\"ios\",\"specific_columns\":\"deviceudid,date_last_beat,tags,lostmode_status,longitude,latitude\"}}"
	#
	# output=$(curl -s -k -X POST -d 'content='$content 'https://managerapi.mosyle.com/v2/listdevices')
	#
	# # echo "What we asked for-->> $content"
